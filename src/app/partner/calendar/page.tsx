// =============================================================
// app/partner/calendar/page.tsx — Partner Calendar Management
// จัดการปฏิทินเฉพาะบ้านของ partner ที่ login อยู่
// =============================================================
...
const PARTNER_CALENDAR_MONTHS = 6;
...
export default function PartnerCalendarPage() {
  ...
  useEffect(() => {
    fetch("/api/partner/properties")
      ...
      .catch(() => setError("โหลดรายการบ้านพักไม่สำเร็จ"))
  ...
  const dateMap = useMemo(() => {
    ...
    availability.forEach((a) =>
      map.set(a.date, { status: a.status, price: a.price, source: a.source })
    );
    return map;
  }, [availability]);
  ...
  const months = useMemo(() => {
    const result = [];
    const now = new Date();
    for (let i = 0; i < PARTNER_CALENDAR_MONTHS; i++) {
  ...
  const handleApplyStatus = async (status: AvailabilityStatus) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(data.error || "บันทึกข้อมูลปฏิทินไม่สำเร็จ");
        return;
      }
      ...
      setError("บันทึกข้อมูลปฏิทินไม่สำเร็จ");
    } finally {
      setSaving(false);
    }
  };
        ...
        >
          ...
          {properties.map((p) => (
            <option key={p.id} value={p.id}>
              {p.property_code ? `${p.property_code} - ${p.name}` : p.name}
            </option>
          ))}
        </select>
      </div>
                  ...
                  {months[mobileMonthIndex].days.map((dayData, idx) => {
                    ...
                    const statusStyle = dayData.isPast
                      ...
                      : dayData.status
                        ...
                        : { bg: "", border: "", text: "text-text-secondary", hover: "hover:bg-card" };
            ...
            <div className="bg-surface rounded-xl border border-gray-200 p-4">
              ...
              <div className="space-y-3">
                ...
                <button
                  ...
                  disabled={selectedDates.size === 0 || saving}
                  className="w-full py-2.5 bg-gray-500/20 text-gray-500 border border-gray-500/30 rounded-lg text-sm font-medium hover:bg-gray-500/30 disabled:opacity-30 transition-colors"
                >
                  ⛔ ทำเครื่องหมาย ปิด
                </button>
